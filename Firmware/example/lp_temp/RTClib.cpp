...
boolean RTC_DS3231::begin(void) {
  Wire.begin();
  //read_i2c_register(DS3231_ADDRESS, DS3231_STATUSREG);
  return true;
}
...
uint8_t RTC_DS3231::temperature(bool start)
{
  uint8_t ctrl;
  uint16_t tempraw;
  uint8_t temp  = 0;
  if (start) {
    ...
    ctrl |= 0x20; //Set temperature conversion bit
    write_i2c_register(DS3231_ADDRESS, DS3231_CONTROL, ctrl);
  } else {
    /*read the temperaure*/
    //tempraw = read_i2c_register(DS3231_ADDRESS, DS3231_TEMPLOWER);
    //tempraw = (read_i2c_register(DS3231_ADDRESS, DS3231_TEMPUPPER) << 8);
    temp = (read_i2c_register(DS3231_ADDRESS, DS3231_TEMPUPPER));
    /*For the time just shift off the decimal place values*/
    /*get rid of sign bit*/
    temp &=0x7F;
  }
  return temp;
